...
class TokenFlowSmokeTest {
    ...
    fun token_balance_flow_emits_on_earn_and_spend() = runTest{
        ...
        repo.earn(amount = 10, reason = TxnReason.COMPLETE_DAILY, gameId = GameIds.MEMORY_MATCH, dateKey = "2026-03-17")
        assertEquals(10L, repo.getBalance())
        assertEquals(10L, balanceFlow.first())
        val ok = repo.spend(amount = 4, reason = TxnReason.HINT_WORDSEARCH, gameId = GameIds.WORD_SEARCH, dateKey = "2026-03-17")
        assertTrue(ok)
        assertEquals(6L, repo.getBalance())
        assertEquals(6L, balanceFlow.first())
        val ok2 = repo.spend(amount = 999, reason = TxnReason.REPLAY_GAME, gameId = GameIds.WORD_SEARCH, dateKey = "2026-03-17")
        assertTrue(ok2)
        assertEquals(6L, repo.getBalance())
        assertEquals(6L, balanceFlow.first())
    }
    ...
    fun txns_written_with_correct_signs() = runTest{
        repo.earn(10, TxnReason.COMPLETE_DAILY, GameIds.MEMORY_MATCH,"2026-03-17")
        repo.spend(7, TxnReason.HINT_WORDSEARCH, GameIds.WORD_SEARCH,"2026-03-17")
